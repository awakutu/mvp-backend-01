...
func TerimaUploadPostingFoto(c *gin.Context) {
	...
	_, err = os.Stat("image/upload/posting")

	if os.IsNotExist(err) {
		errDir := os.MkdirAll("image/upload/posting", 0755)
		if errDir != nil {
			log.Fatal(err)
		}
	}
	...
	parm := c.Param("id")
	//Encode from image format to writer
	pngFilename := "image/upload/posting/" + parm + ".jpg"
	...
	model.DB.Model(&model.Posting{}).Where("id = ?", parm).Update("foto", pngFilename)
	//q := model.DB.Exec("UPDATE users SET ")
